...
func TestConfigManager_EnsureHomeDirExists(t *testing.T) {
	...
	assert.Equal(t, filepath.Join(homeDir, blastHomeDir), c.blastHomeDir)

	fileInfo, err := fs.Stat(c.blastHomeDir)
	assert.NoError(t, err)
...
func TestConfigManager_EnsureVirtualenvDirExists(t *testing.T) {
	...
	fileInfo, err := fs.Stat(filepath.Join(c.blastHomeDir, virtualEnvsPath))
	assert.NoError(t, err)
	assert.True(t, fileInfo.IsDir())
